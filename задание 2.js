async function getCurrentWeatherByyCity(city) {
    const data = await fetch(`http://api.weatherapi.com/v1/current.json?key=66f267600c9f49ef862133628251612&q=${city}&aqi=no`)
...
locationButton.addEventListener('click', async () => {
    ...
    const currentWeather = await getCurrentWeatherByyCity(locationInputValue)
    const forecast = await getForecastByyCity(locationInputValue)
...
async function getForecastByyCity(city) {
    const data = await fetch(`https://api.weatherapi.com/v1/forecast.json?key=66f267600c9f49ef862133628251612&q=${city}&days=1&aqi=no&alerts=no`)
